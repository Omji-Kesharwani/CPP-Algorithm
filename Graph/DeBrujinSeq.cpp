...
string de_bruijn(int k,int n )
{
  ...
  // 'nextEdge[v]' is how many outgoing edges from v we'have already used (range 0..k)

  vector<int>nextEdge(numNodes,0);

  // we'll stimulate a stack of pairs (node,digitUsedToEnterThisNode)
...
while(!stack.empty())
{
  ...
  if(nextEdge[v]<k)
  {
    // take one unused outgoing edge labelled 'd'
    int d = nextEdge[v]++;
